...
initBook(id: any) {
  ...
  onSubmit() {
    ...
    if (!this.validateBook()) {
      ...
      const selectedCategory = this.categories.find(category => category.id === this.book.categoryDTO);
      if (selectedCategory) {
        this.book.categoryDTO = selectedCategory;
